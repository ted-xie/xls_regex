...
const MAX_CYCLES_INNER = u32:1;
const NUM_STATES = u32:8;
type report_t = bits[NUM_REPORTS];
type alphabet_t = bits[256];
type connect_t = bits[NUM_STATES];

fn donut_inner (stream: u8[MAX_CYCLES_INNER], active_curr: u1[NUM_STATES])
    -> (report_t[MAX_CYCLES_INNER], u1[NUM_STATES]) {
  ...
  for (i, (active_curr, result)):
    (u32, (u1[NUM_STATES], report_t[MAX_CYCLES_INNER])) in
      range(u32:0, MAX_CYCLES_INNER) {
      let ch = stream[i];
      //let _ = trace!(i);
      //let _ = trace!(ch);
      ...
      (active_curr, result)
  } ((active_curr, report_t[MAX_CYCLES_INNER]:[report_t:0, ...]));

  (result, active_curr)
}

fn donut_loop (stream: u8[MAX_CYCLES]) -> report_t[MAX_CYCLES] {
  let active_curr = u1[NUM_STATES]:[u1:1, u1:0, ...];
  let (active_curr, result) =
  for (i, (active_curr, result)):
    (u32, (u1[NUM_STATES], report_t[MAX_CYCLES])) in
      ...
      let _ = trace!(ch);
      let _ = trace!(active_curr);
      let (result_i, active_curr) = donut_inner(ch_vec, active_curr);
      let result = update(result, i, result_i[u32:0]);
      (active_curr, result)
...
#![test]
fn donut_test() {
  let result = donut_loop(u8[MAX_CYCLES]:['d', 'o', 'n', 'u',  't', 0, ...]);
